...
def clean_text(raw_text):
    ...
    cleaned_text = re.sub(r'\*{5}.*', '', raw_text, flags=re.DOTALL)

    # Remove content from beginning to "by PIB Delhi"
    cleaned_text = re.sub(r'.*?by PIB Delhi', '', cleaned_text, flags=re.DOTALL)
...
def extract_links_and_text(html_content):
    ...
    date_time_match = re.search(r'(\d{1,2} [A-Z]+ \d{4} \d{1,2}:\d{2}[APM]+) by ([A-Z\s]+)', cleaned_text)
    ...
    return assets, cleaned_text, date, time, department
