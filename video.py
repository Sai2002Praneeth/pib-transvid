import streamlit as st
from moviepy.editor import VideoFileClip
from streamlit_marquee import streamlit_marquee
from generate_video import edit_video_with_voiceover

# Set the path for voiceover and image folders
voiceover_path = 'assets/output.mp3'
scrapped_images_folder = 'Scrapped_Images'
static_images_folder = 'static'

# Set the output video path
output_video_path = 'assets/generated_video.mp4'
txt_file_path = 'assets/v.txt'

def get_video_duration(video_path):
    # Get the duration of the video using MoviePy
    video_clip = VideoFileClip(video_path)
    duration = video_clip.duration
    video_clip.close()
    return duration

def display_text_content(txt_file_path, video_path):
    # Read the content of v.txt
    with open(txt_file_path, 'r', encoding='utf-8') as txt_file:
        text_content = txt_file.read()

    # Get the duration of the video
    video_duration = get_video_duration(video_path)

    # Set marquee duration equal to the video duration
    marquee_duration = video_duration

    # Display the content of v.txt with a custom marquee component
    streamlit_marquee(**{
        'background': "black",
        'font-size': '14px',
        'color': "#ffffff",
        'content': text_content,
        'width': '600px',
        'lineHeight': "25px",
        'animationDuration': f'{marquee_duration}s',
    })

def main():
    st.title("Video Generation Page")
    st.markdown("""
    ### Video Generation Steps:
    1. **Enter Press Release Text:** In the text area below, input the press release text you want to convert into a video.
    2. **Click 'Generate Video':** Once you've entered the text, click the 'Generate Video' button to initiate the video generation process.
    3. **Wait for Processing:** The app will process the text, convert it into spoken words, and integrate visuals to create the video.
    4. **Video Generation Complete:** After processing is complete, you'll receive a message indicating that the video has been generated.

    Ready to create your video? Follow the steps below:
""")
    st.sidebar.title("Video generation")
    st.markdown(
        f"""
            <style>
                [data-testid="stSidebar"] {{
                    background-image: url(data:image/png;base64,);
                    background-repeat: no-repeat;
                    padding-top: 190px;
                    background-position: 20px 20px;
                    width: 10px;
                }}
            </style>
            """,
        unsafe_allow_html=True,
    )
    # Sidebar navigation
    st.sidebar.header("Press Release Video Generation App!")

    # Button to generate the video
    if st.sidebar.button("Generate Video"):
        st.info("Video generation in progress...")

        # Generate the video and get the path
        generated_video_path = edit_video_with_voiceover(voiceover_path, scrapped_images_folder, static_images_folder)

        if generated_video_path:
            st.success("Video generated successfully!")

            # Display the content of v.txt with marquee duration equal to the video duration
            display_text_content(txt_file_path, generated_video_path)

            # Display the generated video
            display_generated_video(generated_video_path)

def display_generated_video(video_path):
    # Read the video file as bytes
    with open(video_path, 'rb') as video_file:
        video_bytes = video_file.read()

    # Display the video
    st.video(video_bytes, format="video/mp4")

if __name__ == "__main__":
    main()
