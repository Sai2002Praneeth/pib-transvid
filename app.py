import streamlit as st
import shutil
import os
import subprocess  # Import the subprocess module
from scrap_press_release import scrape_web_page

def main():
    st.title("Press Release Video Generation App!")
    st.markdown(
        f"""
            <style>
                [data-testid="stSidebar"] {{
                    background-image: url(data:image/png;base64,);
                    background-repeat: no-repeat;
                    padding-top: 190px;
                    background-position: 20px 20px;
                    width: 10px;
                }}
            </style>
            """,
        unsafe_allow_html=True,
    )
    # Sidebar navigation
    st.sidebar.header("Press Release Video Generation App!")
    st.sidebar.header("Navigation")
    page_options = ["Home", "Scrape Press Release", "Clear Cache", "Run Clean Script"]
    page_choice = st.sidebar.selectbox("Select Page", page_options)

    if page_choice == "Home":
        display_home()
    elif page_choice == "Scrape Press Release":
        display_scrape_page()
    elif page_choice == "Clear Cache":
        display_clear_cache()
    elif page_choice == "Run Clean Script":  # Added a new option
        run_clean_script()

def display_home():
    st.write("Welcome to the Press Release Video Generation App!")
    st.write("""
    The main objective of this project is to automate the process of converting press releases into compelling videos. 
    By leveraging text-to-speech (TTS) technology and incorporating relevant visuals, this app streamlines the 
    video generation process.

    ## Key Features:
    - **Text-to-Speech (TTS):** Convert press release text into spoken words.
    - **Visual Enhancement:** Integrate images or video clips to accompany the narration.
    - **Customization:** Personalize the video with various styles and themes.
    - **Export Options:** Save or share the generated videos easily.

    ### How it Works:
    1. **Input:** Provide the press release text.
    2. **Text-to-Speech:** The app converts the text into spoken words.
    3. **Visual Integration:** Enhance the video with relevant images or video clips.
    4. **Customization:** Customize the video style and theme.
    5. **Export:** Save or share your generated video!

    Feel free to explore and experiment with turning your press releases into engaging videos. If you have any feedback or suggestions, 
    we'd love to hear from you. Enjoy creating your multimedia content with our Press Release to Video Generation App!
""")

def display_scrape_page():
    st.header("Scrape Press Release")
    url_to_scrape = st.text_input("Enter the URL to scrape:")

    if st.button("Scrape Press Release"):
        if not url_to_scrape:
            st.warning("Please enter a valid URL.")
        else:
            st.info("Scraping in progress... Please wait.")
            scrape_web_page(url_to_scrape)
            st.success("Scraping completed successfully!")

            # Display results or additional information
            display_results()

def display_results():
    # You can customize this function to display relevant information or visualizations.
    st.header("Scraping Results")

    # Display text content
    with st.expander("Text Content"):
        with open('assets/webpage_text.txt', 'r', encoding='utf-8') as text_file:
            text_content = text_file.read()
            st.text(text_content)

    # Display links
    with st.expander("Links on the Page"):
        with open('assets/webpage_links.txt', 'r', encoding='utf-8') as links_file:
            links = links_file.readlines()
            for link in links:
                st.markdown(f"- {link.strip()}")

    # Display images
    image_paths = [f"Scrapped_Images/{img}" for img in os.listdir('Scrapped_Images')]
    with st.expander("Downloaded Images"):
        for img_path in image_paths:
            st.image(img_path, caption=os.path.basename(img_path), use_column_width=True)

def display_clear_cache():
    # Clear the Scrapped_Images folder
    shutil.rmtree('Scrapped_Images', ignore_errors=True)

    # Clear the assets folder
    shutil.rmtree('assets', ignore_errors=True)

    # Recreate the folders
    os.mkdir('Scrapped_Images')
    os.mkdir('assets')

    st.success("Cache cleared successfully!")

def run_clean_script():
    st.header("Run Clean Script")

    if st.button("Run Clean Script"):
        st.info("Running clean_script.py... Please wait.")
        
        # Use subprocess to run the clean_script.py
        subprocess.run(["streamlit", "run", "clean_script.py"])

        st.success("Clean Script executed successfully!")


if __name__ == "__main__":
    main()
